...
class TriDet(nn.Module):
    ...
    def __init__(
        ...
        self.transconv1d=torch.nn.ConvTranspose1d(in_channels=self.num_classes,out_channels=self.num_classes,kernel_size=13,stride=2,output_padding=1,padding=6)
    ...
    def forward(self, video_list):
        ...
        if self.use_trident_head:
            ...
            trans_feats_out_lb=[self.transconv1d(i) for i in out_lb_logits]
            trans_feats_out_rb=[self.transconv1d(i) for i in out_rb_logits]

            new_feats_out_lb_logits,new_feats_out_rb_logits=[],[]
            for index in range(len(out_lb_logits)):
                if index<len(out_lb_logits)-1:
                    new_feats_out_lb_logits.append(out_lb_logits[index]+trans_feats_out_lb[index+1])
                    new_feats_out_rb_logits.append(out_rb_logits[index]+trans_feats_out_rb[index+1])
                else:
                    new_feats_out_lb_logits.append(out_lb_logits[index])
                    new_feats_out_rb_logits.append(out_rb_logits[index])

            out_lb_logits = new_feats_out_lb_logits
            out_rb_logits = new_feats_out_rb_logits
    ...
    def losses(
        ...
        T_i = [i.size(1) for i in fpn_masks]
        gt_cls_i=gt_cls.split(T_i,dim=1)
        gt_offsets_i=gt_offsets.split(T_i,dim=1)
        pos_mask_i=[]
        for cls,val_mask in zip(gt_cls_i,fpn_masks):
            pos_mask_i.append(torch.logical_and((cls.sum(-1)>0),val_mask))
        num_pos_i=[i.sum().item() for i in pos_mask_i]
        min_pos_samples = min(num_pos_i)
        max_pos_samples = max(num_pos_i)
        interval = 1. / (max_pos_samples - min_pos_samples + 1e-10)
        reweight_factor_per_level_i = []
        for pos_samples in num_pos_i:
            factor = 2. - (pos_samples - min_pos_samples) * interval
            reweight_factor_per_level_i.append(factor)
        ...
        decoded_offsets_i=decoded_offsets.split(T_i,dim=1)
        reg_loss_i=[]
        cls_loss_i=[]

        
        for posmask,decodedoffsets,gtclss,gtoffset,out_cls_logit,val_mask,reweight_factor_per_level in zip(pos_mask_i,decoded_offsets_i,gt_cls_i,gt_offsets_i,out_cls_logits,fpn_masks,reweight_factor_per_level_i):
            decodedoffsets = decodedoffsets[posmask]
            if self.use_trident_head:
                # the boundary head predicts the classification score for each categories.
                pred_offsets = decodedoffsets[gtclss[posmask].bool()]
                # cat the predicted offsets -> (B, FT, 2 (xC)) -> # (#Pos, 2 (xC))
                vid = torch.where(gtclss[posmask])[0]
                gtoffsets = gtoffset[posmask][vid]
            else:
                pred_offsets = decodedoffsets
                gtoffsets = gtoffset[posmask]

            # update the loss normalizer
            num_pos = posmask.sum().item()
            self.loss_normalizer = self.loss_normalizer_momentum * self.loss_normalizer + (
                    1 - self.loss_normalizer_momentum
            ) * max(num_pos, 1)

            # gt_cls is already one hot encoded now, simply masking out
            gt_target = gtclss[val_mask]
            ...
            cls_loss = reweight_factor_per_level*cls_loss.sum()
            ...
            if num_pos == 0:
                ...
                reg_loss=reweight_factor_per_level*reg_loss
            ...
            cls_loss_i.append(cls_loss)
            reg_loss_i.append(reg_loss)

        cls_cost,reg_cost=sum(cls_loss_i),sum(reg_loss_i)
        # return a dict of losses
        final_loss = cls_cost + reg_cost * loss_weight
        return {'cls_loss': cls_cost,
                'reg_loss': reg_cost,
